# Crear un programa que simule un sistema de gestión de votos de manera aleatoria.
import random
...
while i != 5:
    ...
    for j in range(3):
        localidades[i][j] = random.randint(0, 300)
    if (localidades[i][0] + localidades[i][1] + localidades[i][2]) <= 300:
        i += 1
    else:
        print("La cantidad de votantes generados es mayor a 300. Reiniciando el ciclo")
